...
The SAM CLI installs dependencies defined in `MatchPage/package.json`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
$ sam local invoke MatchPage --event events/event.json
```
...
```bash
$ npm run build
$ sam deploy --stack-name MatchPage --capabilities CAPABILITY_IAM --s3-bucket tranmere-web-api
```
...
```bash
$ sam logs -n MatchPageFunction --stack-name MatchPage --tail
```
...
```bash
$ aws cloudformation delete-stack --stack-name MatchPage
```
